...
export default function EarningTypes({ earningTypes, categories, calculationTypes }: Props) {
    const [showModal, setShowModal] = useState(false);
    const [editingType, setEditingType] = useState<EarningTypeModel | null>(null);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingType) {
            form.put(
                PayrollSettingsController.updateEarningType.url({ earningType: editingType.id }),
                {
    ...
    const handleDelete = (id: number) => {
        ...
        router.delete(PayrollSettingsController.destroyEarningType.url({ earningType: id }), {
            onSuccess: () => setDeleteConfirm(null),
            onError: (errors) => {
                setDeleteConfirm(null);
                setErrorMessage(Object.values(errors).flat().join(', ') || 'Failed to delete earning type');
            },
        });
    };
            ...
            <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        Configure the types of earnings that can be assigned to employees
                    </p>
                </div>
                    ...
                    <Card className="max-h-[90vh] w-full max-w-lg overflow-y-auto p-4 sm:p-6">
                        <div className="mb-4 flex items-center justify-between">
                            <h3 className="text-base font-semibold text-gray-900 dark:text-white sm:text-lg">
                                {editingType ? 'Edit Earning Type' : 'Add Earning Type'}
                            </h3>
                            ...
                            <div className="space-y-3">
                                <div className="flex items-center gap-3">
                                    <Checkbox
                                        ...
                                        onChange={(e) => form.setData('is_taxable', e.target.checked)}
                                    />
                                    <Label htmlFor="is_taxable" className="mb-0">
                                ...
                                <div className="flex items-center gap-3">
                                    <Checkbox
                                        ...
                                        onChange={(e) => form.setData('is_pensionable', e.target.checked)}
                                    />
                                    <Label htmlFor="is_pensionable" className="mb-0">
                                        Pensionable (subject to pension contributions)
                                    </Label>
                                </div>
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <Label htmlFor="is_recurring" className="mb-0">
                                        Recurring (automatically included each pay period)
                                    </Label>
                                </div>
