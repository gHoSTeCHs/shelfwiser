...
export default function Create({
    ...
    const [selectedShop, setSelectedShop] = useState<string>(shops[0]?.id.toString() || '');
    const [selectedSupplier, setSelectedSupplier] = useState<string>(
        selectedSupplierId?.toString() || ''
    );
    const [catalog, setCatalog] = useState<SupplierCatalogItem[]>(supplierCatalog);
    const [cart, setCart] = useState<CartItem[]>([]);
    const [loadingCatalog, setLoadingCatalog] = useState(false);

    const { data, setData, post, processing, errors } = useForm({
        shop_id: selectedShop,
        supplier_tenant_id: selectedSupplier,
        items: [] as { catalog_item_id: number; quantity: number; unit_price: number }[],
        expected_delivery_date: '',
        notes: '',
    });

    useEffect(() => {
        if (selectedSupplier && selectedSupplier !== data.supplier_tenant_id) {
            setLoadingCatalog(true);
            router.get(
                route('purchase-orders.create', { supplier: selectedSupplier }),
                {},
                {
                    preserveState: true,
                    preserveScroll: true,
            ...
            setData('supplier_tenant_id', selectedSupplier);
        }
    }, [selectedSupplier]);

    useEffect(() => {
        setData('shop_id', selectedShop);
    }, [selectedShop]);

    const calculateItemPrice = (item: SupplierCatalogItem, quantity: number): number => {
    ...
    const addToCart = (catalogItem: SupplierCatalogItem) => {
        const existingItem = cart.find((item) => item.catalog_item_id === catalogItem.id);

        if (existingItem) {
            updateQuantity(catalogItem.id, existingItem.quantity + catalogItem.min_order_quantity);
        } else {
    ...
    const updateQuantity = (catalogItemId: number, newQuantity: number) => {
        const updatedCart = cart.map((item) => {
            if (item.catalog_item_id === catalogItemId) {
                const unit_price = calculateItemPrice(item.catalog_item, newQuantity);
            ...
            <AppLayout>
                ...
                <div className="space-y-6">
                    <EmptyState
                        ...
                        action={
                            <Button onClick={() => router.visit(route('supplier.connections.index'))}>
                                View Suppliers
                            </Button>
                        }
                    />
                    ...
                    <div className="lg:col-span-2">
                        <Card className="p-6">
                            ...
                            <div className="space-y-4">
                                <div>
                                    <Label htmlFor="shop_id" required>
                                        Receiving Shop
                                    </Label>
                                    <Select
                                    ...
                                    />
                                    {errors.shop_id && (
                                        <p className="mt-1 text-sm text-red-600">{errors.shop_id}</p>
                                    )}
                                </div>

                                <div>
                                    <Label htmlFor="supplier_tenant_id" required>
                                        Supplier
                                    </Label>
                                    <Select
                                        options={supplierConnections.map((conn) => ({
                                            value: conn.supplier_tenant_id.toString(),
                                            label: conn.supplier_tenant?.name || 'Unknown',
                                        }))}
                                                ...
                                                >
                                                    <div className="flex-1">
                                                        ...
                                                        <div className="mt-1 flex items-center gap-4 text-sm text-gray-500 dark:text-gray-400">
                                                            <span>
                                                                Base: ${item.base_wholesale_price.toFixed(2)}
                                                            </span>
                                                            <span>
                                                    ...
                                                    <Button
                                                        ...
                                                        disabled={cart.some(
                                                            (cartItem) =>
                                                                cartItem.catalog_item_id === item.id
                                                        )}
                                                    >
                                ...
                                <>
                                                ...
                                                <div className="flex items-start justify-between">
                                                    <div className="flex-1">
                                                        <h4 className="text-sm font-medium text-gray-900 dark:text-white">
                                                            {item.catalog_item.product?.name}
                                                        </h4>
                                    ...
                                    <div className="mt-4 space-y-2 border-t pt-4 dark:border-gray-700">
                                        ...
                                        <p className="text-xs text-gray-500 dark:text-gray-400">
                                            Tax and shipping calculated by supplier
                                        </p>
                                    </div>
