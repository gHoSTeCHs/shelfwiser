...
class ReportService
{
    ...
    public function getInventoryReport(
        ...
        if (in_array($stockStatus, ['low', 'adequate', 'overstocked'], true)) {
            $shopIdList = $shopIds->all();
            $placeholders = implode(',', array_fill(0, count($shopIdList), '?'));
            $shopClass = Shop::class;
            $stockSubquery = "(SELECT COALESCE(SUM(quantity), 0) FROM inventory_locations WHERE product_variant_id = product_variants.id AND location_type = ? AND location_id IN ($placeholders))";
            $bindings = array_merge([$shopClass], $shopIdList);

            $query->whereNotNull('reorder_level');

            if ($stockStatus === 'low') {
                $query->whereRaw("$stockSubquery <= reorder_level", $bindings);
            } elseif ($stockStatus === 'adequate') {
                $query->whereRaw("$stockSubquery > reorder_level", $bindings)
                    ->whereRaw("$stockSubquery <= (reorder_level * 3)", $bindings);
            } else { // overstocked
                $query->whereRaw("$stockSubquery > (reorder_level * 3)", $bindings);
            }
        }
    ...
    public function getInventorySummary(Collection $shopIds): array
    {
        ...
        return Cache::remember($cacheKey, now()->addMinutes(15), function () use ($shopIds) {
            ...
            $placeholders = implode(',', array_fill(0, count($shopIdList), '?'));
            $stockSubquery = "(SELECT COALESCE(SUM(quantity), 0) FROM inventory_locations WHERE product_variant_id = product_variants.id AND location_type = ? AND location_id IN ($placeholders))";
            $bindings = array_merge([Shop::class], $shopIdList);

            $lowStockCount = ProductVariant::whereHas('product', function ($q) use ($shopIds) {
                $q->whereIn('shop_id', $shopIds);
            })
                ->whereNotNull('reorder_level')
                ->whereRaw("$stockSubquery <= reorder_level", $bindings)
                ->count();
    ...
    public function getCustomerAnalyticsSummary(Collection $shopIds, Carbon $startDate, Carbon $endDate): array
    {
        ...
        return Cache::remember($cacheKey, now()->addMinutes(30), function () use ($shopIds, $startDate, $endDate) {
            ...
            $highValueCustomers = Order::query()
                ->select('customer_id')
